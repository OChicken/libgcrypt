...
static int test_keys (ECELG_secret_key *sk, unsigned int nbits, int nodie);
static gcry_mpi_t gen_k (gcry_mpi_t p);
static gcry_err_code_t generate (ECELG_secret_key *sk, unsigned nbits,
                                 gcry_mpi_t **factors);
static int  check_secret_key (ECELG_secret_key *sk);
static void do_encrypt (gcry_mpi_t a, gcry_mpi_t b, gcry_mpi_t input,
                        ECELG_public_key *pkey);
static void decrypt (gcry_mpi_t output, gcry_mpi_t a, gcry_mpi_t b,
                     ECELG_secret_key *skey);
static unsigned int ecelg_get_nbits (gcry_sexp_t parms);
...
void
_gcry_register_pk_ecelg_progress (void (*cb) (void *, const char *,
                                            int, int, int),
				void *cb_data)
{
  progress_cb = cb;
...
{
  if (progress_cb)
    progress_cb (progress_cb_data, "pk_ecelg", c, 0, 0);
}
...
static int
test_keys ( ECELG_secret_key *sk, unsigned int nbits, int nodie )
{
  ECELG_public_key pk;
...
generate ( ECELG_secret_key *sk, unsigned int nbits, gcry_mpi_t **ret_factors )
{
  gcry_err_code_t rc;
  ...
  if( DBG_CIPHER )
    {
      progress ('\n');
      log_mpidump ("ecelg  p", p );
      log_mpidump ("ecelg  g", g );
      log_mpidump ("ecelg  y", y );
      log_mpidump ("ecelg  x", x );
    }
...
static gcry_err_code_t
generate_using_x (ECELG_secret_key *sk, unsigned int nbits, gcry_mpi_t x,
 ...
 */
static int
check_secret_key( ECELG_secret_key *sk )
{
  int rc;
...
static void
do_encrypt(gcry_mpi_t a, gcry_mpi_t b, gcry_mpi_t input, ECELG_public_key *pkey )
{
  gcry_mpi_t k;
  ...
  if( DBG_CIPHER )
    {
      log_mpidump("ecelg encrypted y", pkey->y);
      log_mpidump("ecelg encrypted p", pkey->p);
      log_mpidump("ecelg encrypted k", k);
      log_mpidump("ecelg encrypted M", input);
      log_mpidump("ecelg encrypted a", a);
      log_mpidump("ecelg encrypted b", b);
    }
#endif
  mpi_free(k);
}
...
static void
decrypt (gcry_mpi_t output, gcry_mpi_t a, gcry_mpi_t b, ECELG_secret_key *skey )
{
  ...
  if( DBG_CIPHER )
    {
      log_mpidump ("ecelg decrypted x", skey->x);
      log_mpidump ("ecelg decrypted p", skey->p);
      log_mpidump ("ecelg decrypted a", a);
      log_mpidump ("ecelg decrypted b", b);
      log_mpidump ("ecelg decrypted M", output);
    }
#endif
  mpi_free (t1);
}
...
ecelg_generate (const gcry_sexp_t genparms, gcry_sexp_t *r_skey)
{
  gpg_err_code_t rc;
  unsigned int nbits;
  ECELG_secret_key sk;
...
static gcry_err_code_t
ecelg_check_secret_key (gcry_sexp_t keyparms)
{
  gcry_err_code_t rc;
  ECELG_secret_key sk = {NULL, NULL, NULL, NULL};
 ...
 leave:
  ...
  if (DBG_CIPHER)
    log_debug ("ecelg_testkey    => %s\n", gpg_strerror (rc));
  return rc;
}


static gcry_err_code_t
ecelg_encrypt (gcry_sexp_t *r_ciph, gcry_sexp_t s_data, gcry_sexp_t keyparms)
{
  gcry_err_code_t rc;
  ...
  ECELG_public_key pk = { NULL, NULL, NULL };

  _gcry_pk_util_init_encoding_ctx (&ctx, PUBKEY_OP_ENCRYPT,
                                   ecelg_get_nbits (keyparms));
  ...
  if (rc)
    goto leave;
  if (DBG_CIPHER)
    log_mpidump ("ecelg_encrypt data", data);
  ...
  if (DBG_CIPHER)
    {
      log_mpidump ("ecelg_encrypt  p", pk.p);
      log_mpidump ("ecelg_encrypt  g", pk.g);
      log_mpidump ("ecelg_encrypt  y", pk.y);
    }
  ...
  rc = sexp_build (r_ciph, NULL, "(enc-val(ecelg(a%m)(b%m)))", mpi_a, mpi_b);
 ...
 leave:
  ...
  if (DBG_CIPHER)
    log_debug ("ecelg_encrypt   => %s\n", gpg_strerror (rc));
  return rc;
}


static gcry_err_code_t
ecelg_decrypt (gcry_sexp_t *r_plain, gcry_sexp_t s_data, gcry_sexp_t keyparms)
{
  ...
  /* Extract the data.  */
  rc = _gcry_pk_util_preparse_encval (s_data, ecelg_names, &l1, &ctx);
  if (rc)
    goto leave;
  ...
  if (DBG_CIPHER)
    {
      log_printmpi ("ecelg_decrypt  d_a", data_a);
      log_printmpi ("ecelg_decrypt  d_b", data_b);
    }
  ...
  if (DBG_CIPHER)
    {
      log_printmpi ("ecelg_decrypt    p", sk.p);
      log_printmpi ("ecelg_decrypt    g", sk.g);
      log_printmpi ("ecelg_decrypt    y", sk.y);
      if (!fips_mode ())
        log_printmpi ("ecelg_decrypt    x", sk.x);
    }

  plain = mpi_snew (ctx.nbits);
  decrypt (plain, data_a, data_b, &sk);
  if (DBG_CIPHER)
    log_printmpi ("ecelg_decrypt  res", plain);
 ...
 */
static unsigned int
ecelg_get_nbits (gcry_sexp_t parms)
{
  gcry_sexp_t l1;
  gcry_mpi_t p;
  unsigned int nbits;
...

gcry_pk_spec_t _gcry_pubkey_spec_ecelg =
  {
    GCRY_PK_ECELG, { 0, 0 },
    (GCRY_PK_USAGE_ENCR),
    "ECELG", ecelg_names,
